...
public class DataHora // declaração da classe 
 ...
 /**
  * O método éIgual recebe uma instância da própria classe DataHora como argumento e
  * verifica se a DataHora representada pela classe e pela instância que foi passada é
  * a mesma. A comparação é feita comparando os campos da DataHora um a um.
  * @param outraDataHora uma instância da própria classe DataHora
  * @return true se a DataHora encapsulada for igual à passada, false caso contrário
  */
  public boolean éIgual(DataHora outraDataHora)
    {
        if(estaData.éIgual(outraDataHora.estaData) && 
            estaHora.éIgual(outraDataHora.estaHora)){
            return true;
        }else{
            return false;
        }
